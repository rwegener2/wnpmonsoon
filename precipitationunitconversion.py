...
# takes a netcdf file of precipitation flux and changes it to either precipitation rate or total precipitation.
# outputs a new netcdf in the specified location of the new values
# precip flux files assumed to be in the same directory "directory_saved_files"
# output files place in new folder created in same location as precip flux directory location
# "I:" denotes an instruction - something to change in order to run the code or a print statement to check
...
# I: place all spatially and temporally subset files in the same folder; write folder name as 'directory_saved_files'
file_list = []
directory_saved_files = r'/snfs1/users/nasa_develop/Fall2017/CMIP5/unprocessed/'+model+'/clipped_concat/'
for dirpath, _, filenames in os.walk(directory_saved_files):
    for f in filenames:
        if f.endswith('.nc') and f.startswith('pr_'):
            abs_path = os.path.abspath(os.path.join(dirpath, f))
            file_list.append(abs_path)
    break

# I: ensure print statement displays the correct number of files
print('Found ', len(file_list), ' files for processing')
